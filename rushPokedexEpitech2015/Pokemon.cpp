...
// Last update Sun Jan 25 19:32:14 2015 Julien Selaries
//
      ...
      if (found != std::string::npos)
	{
	  ...
	  if (found != std::string::npos)
	      ...
	      if (i != 0)
		{
		  bufferfind = found;
		  found = xml.find("<lvl>", found);
		  bufferfind = xml.find("<condition>", bufferfind);
		  if ((bufferfind < found || found == std::string::npos) && bufferfind != std::string::npos)
		    {
		      bufferfind+= 13;
		      evo[i] += " at condition ";
		      while (xml[bufferfind] != '<')
			evo[i] += xml[bufferfind++];
		      found = bufferfind;
		    }
